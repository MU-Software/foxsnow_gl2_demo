...
static ImageRec* ImageOpen(char* fileName) {
	...
	if ((image->type & 0xFF00) == 0x0100) {
		...
		if (swapFlag) {
			...
			ConvertLong((unsigned int*)image->rowSize, x / sizeof(int));
		}
	}
	return image;
}
...
GLuint loadTEX(char* name) {
	...
	int components = image->zsize;
	base = (unsigned int*)malloc(image->xsize * image->ysize * sizeof(unsigned));
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, (GLvoid*)base);
